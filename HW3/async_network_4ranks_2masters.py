...
class AsynchronicNeuralNetwork(NeuralNetwork):
    ...
    def do_worker(self, training_data):
        ...
        for epoch in range(self.epochs):
            ...
            for minibatch_idx, (x, y) in enumerate(mini_batches):
                ...
                # print(f'worker {self.rank} send nabla_w[1] to master 1: {nabla_w[1][0,0]:.4f}...{nabla_w[1][-1,-1]:.4f} ({nabla_w[1].shape})')
                self.comm.Isend(nabla_w[1], 1)
                ...
                # print(f'worker {self.rank} send nabla_w[3] to master 1: {nabla_w[3][0,0]:.4f}...{nabla_w[3][-1,-1]:.4f} ({nabla_w[3].shape})')
                self.comm.Isend(nabla_w[3], 1)

                # print(f'worker {self.rank} send nabla_b[0] to master 0: {nabla_b[0][0]:.4f} ({nabla_b[0].shape})')
                self.comm.Isend(nabla_b[0], 0)
                # print(f'worker {self.rank} send nabla_b[1] to master 1: {nabla_b[1][0]:.4f}...{nabla_b[1][-1]:.4f} ({nabla_b[1].shape})')
                self.comm.Isend(nabla_b[1], 1)
                ...
                # print(f'worker {self.rank} send nabla_b[3] to master 1: {nabla_b[3][0]:.4f}...{nabla_b[3][-1]:.4f} ({nabla_b[3].shape})')
                self.comm.Isend(nabla_b[3], 1)
                ...
                # print(f'worker {self.rank} weights[1] before recv from master 1: {self.weights[1][0,0]:.4f}...{self.weights[1][-1,-1]:.4f} ({self.weights[1].shape})')
                # print(f'worker {self.rank} biases[0]  before recv from master 0: {self.biases[0][0]:.4f} ({self.biases[0].shape})')
                # print(f'worker {self.rank} biases[1]  before recv from master 1: {self.biases[1][0]:.4f}...{self.biases[1][-1]:.4f} ({self.biases[1].shape})')
                ...
                # print(f'worker {self.rank} weights[3] before recv from master 1: {self.weights[3][0,0]:.4f}...{self.weights[3][-1,-1]:.4f} ({self.weights[3].shape})')
                # print(f'worker {self.rank} biases[2]  before recv from master 0: {self.biases[2][0]:.4f}...{self.biases[2][-1]:.4f} ({self.biases[2].shape})')
                # print(f'worker {self.rank} biases[3]  before recv from master 1: {self.biases[3][0]:.4f}...{self.biases[3][-1]:.4f} ({self.biases[3].shape})')
                recv_w_req0 = self.comm.Irecv(self.weights[0], 0)
                recv_w_req1 = self.comm.Irecv(self.weights[1], 1)
                recv_w_req2 = self.comm.Irecv(self.weights[2], 0)
                recv_w_req3 = self.comm.Irecv(self.weights[3], 1)

                recv_b_req0 = self.comm.Irecv(self.biases[0], 0)
                recv_b_req1 = self.comm.Irecv(self.biases[1], 1)
                recv_b_req2 = self.comm.Irecv(self.biases[2], 0)
                recv_b_req3 = self.comm.Irecv(self.biases[3], 1)
                ...
                # print(f'worker {self.rank} weights[1] after  recv from master 1: {self.weights[1][0,0]:.4f}...{self.weights[1][-1,-1]:.4f} ({self.weights[1].shape})')
                ...
                # print(f'worker {self.rank} weights[3] after  recv from master 1: {self.weights[3][0,0]:.4f}...{self.weights[3][-1,-1]:.4f} ({self.weights[3].shape})')
                # print(f'worker {self.rank} biases[0]  after  recv from master 0: {self.biases[0][0]:.4f} ({self.biases[0].shape})')
                # print(f'worker {self.rank} biases[1]  after  recv from master 1: {self.biases[1][0]:.4f}...{self.biases[1][-1]:.4f} ({self.biases[1].shape})')
                # print(f'worker {self.rank} biases[2]  after  recv from master 0: {self.biases[2][0]:.4f}...{self.biases[2][-1]:.4f} ({self.biases[2].shape})')
                # print(f'worker {self.rank} biases[3]  after  recv from master 1: {self.biases[3][0]:.4f}...{self.biases[3][-1]:.4f} ({self.biases[3].shape})')
    ...
    def do_master(self, validation_data):
        ...
        for epoch in range(self.epochs):
            for batch in range(self.number_of_batches):
                ...
                if self.rank == 0:  # master 0
                    ...
                    self.weights[2] = self.weights[2] - self.eta * nabla_w[1]
                    
                    self.biases[0] = self.biases[0] - self.eta * nabla_b[0]
                    self.biases[2] = self.biases[2] - self.eta * nabla_b[1]
                ...
                elif self.rank == 1:  # master 1 
                    # print(f'Master 1 nabla_w[0] before recv from Worker {worker_idx}: {nabla_w[0][0,0]:.4f}...{nabla_w[0][-1,-1]:.4f} ({nabla_w[0].shape})')  
                    recv_nab_w_req0 = self.comm.Irecv(nabla_w[0], worker_idx)
                    recv_nab_w_req0.Wait()
                    # print(f'Master 1 nabla_w[0] after  recv from Worker {worker_idx}: {nabla_w[0][0,0]:.4f}...{nabla_w[0][-1,-1]:.4f} ({nabla_w[0].shape})')
                    # print(f'Master 1 nabla_w[1] before recv from Worker {worker_idx}: {nabla_w[1][0,0]:.4f}...{nabla_w[1][-1,-1]:.4f} ({nabla_w[1].shape})')  
                    recv_nab_w_req1 = self.comm.Irecv(nabla_w[1], worker_idx)
                    recv_nab_w_req1.Wait()
                    # print(f'Master 1 nabla_w[1] after  recv from Worker {worker_idx}: {nabla_w[1][0,0]:.4f}...{nabla_w[1][-1,-1]:.4f} ({nabla_w[1].shape})')

                    # print(f'Master 1 nabla_b[0] before recv from Worker {worker_idx}: {nabla_b[0][0]:.4f}...{nabla_b[0][-1]:.4f} ({nabla_b[0].shape})')  
                    recv_nab_b_req0 = self.comm.Irecv(nabla_b[0], worker_idx)
                    recv_nab_b_req0.Wait()
                    # print(f'Master 1 nabla_b[0] after  recv from Worker {worker_idx}: {nabla_b[0][0]:.4f}...{nabla_b[0][-1]:.4f} ({nabla_b[0].shape})')
                    
                    # print(f'Master 1 nabla_b[1] before recv from Worker {worker_idx}: {nabla_b[1][0]:.4f}...{nabla_b[1][-1]:.4f} ({nabla_b[1].shape})')  
                    recv_nab_b_req1 = self.comm.Irecv(nabla_b[1], worker_idx)
                    recv_nab_b_req1.Wait()
                    # print(f'Master 1 nabla_b[1] after  recv from Worker {worker_idx}: {nabla_b[1][0]:.4f}...{nabla_b[1][-1]:.4f} ({nabla_b[1].shape})')
                    
                    self.weights[1] = self.weights[1] - self.eta * nabla_w[0]
                    self.weights[3] = self.weights[3] - self.eta * nabla_w[1]

                    self.biases[1] = self.biases[1] - self.eta * nabla_b[0]
                    self.biases[3] = self.biases[3] - self.eta * nabla_b[1]

                    # print(f'Master 1 send weights[1] to Worker {worker_idx}: {self.weights[1][0,0]:.4f}...{self.weights[1][-1,-1]:.4f} ({self.weights[1].shape})')
                    self.comm.Isend(self.weights[1], worker_idx)
                    # print(f'Master 1 send weights[3] to Worker {worker_idx}: {self.weights[3][0,0]:.4f}...{self.weights[3][-1,-1]:.4f} ({self.weights[3].shape})')
                    self.comm.Isend(self.weights[3], worker_idx)
                    # print(f'Master 1 send biases[1]  to Worker {worker_idx}: {self.biases[1][0]:.4f}...{self.biases[1][-1]:.4f} ({self.biases[1].shape})')
                    self.comm.Isend(self.biases[1], worker_idx)
                    # print(f'Master 1 send biases[3]  to Worker {worker_idx}: {self.biases[3][0]:.4f}...{self.biases[3][-1]:.4f} ({self.biases[3].shape})')
        ...
        if self.rank != 0:
            for i in range(self.rank, self.num_layers, self.num_masters):
                # print(f'Master {self.rank} sends weights[{i}] to master 0:', end=' ')
                # print(f'{self.weights[i][0]} ({self.weights[i].shape})') if i == 0 else print(f'{self.weights[i][0,0]}...{self.weights[i][-1,-1]} ({self.weights[i].shape})')
                send_w_req = self.comm.Isend(self.weights[i], 0)
                # send_w_req.Wait()
            for i in range(self.rank, self.num_layers, self.num_masters):
                # print(f'Master {self.rank} sends biases[{i}] to master 0:', end=' ')
                # print(f'{self.biases[i][0]} ({self.biases[i].shape})') if i == 0 else print(f'{self.biases[i][0]}...{self.biases[i][-1]} ({self.biases[i].shape})')
                send_b_req = self.comm.Isend(self.biases[i], 0)
                # send_b_req.Wait()
        if self.rank == 0:
            # iterate through all other masters
            for master_idx in range(1, self.num_masters):
                for i in range(master_idx, self.num_layers, self.num_masters):
                    # print(f'Master 0 weights[{i}] before recv from master {master_idx}:', end=' ')
                    # print(f'{self.weights[i][0]} ({self.weights[i].shape})') if i == 0 else print(f'{self.weights[i][0,0]}...{self.weights[i][-1,-1]} ({self.weights[i].shape})')
                    recv_w_req = self.comm.Irecv(self.weights[i], master_idx)
                    recv_w_req.Wait()
                    # print(f'Master 0 weights[{i}] after  recv from master {master_idx}:', end=' ')
                    # print(f'{self.weights[i][0]} ({self.weights[i].shape})') if i == 0 else print(f'{self.weights[i][0,0]}...{self.weights[i][-1,-1]} ({self.weights[i].shape})')
                for i in range(master_idx, self.num_layers, self.num_masters):
                    # print(f'Master 0 biases[{i}] before recv from master {master_idx}:', end=' ')
                    # print(f'{self.biases[i][0]} ({self.biases[i].shape})') if i == 0 else print(f'{self.biases[i][0]}...{self.biases[i][-1]} ({self.biases[i].shape})')
                    recv_b_req = self.comm.Irecv(self.biases[i], master_idx)
                    recv_b_req.Wait()
                    # print(f'Master 0 biases[{i}] after  recv from master {master_idx}:', end=' ')
                    # print(f'{self.biases[i][0]} ({self.biases[i].shape})') if i == 0 else print(f'{self.biases[i][0]}...{self.biases[i][-1]} ({self.biases[i].shape})')
